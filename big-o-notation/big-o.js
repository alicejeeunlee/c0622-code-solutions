...
function uniqueLinear(words) {
  ...
  for (
    ...
    const word = words[i];          // 2 * n = O(2n) ~= O(n) -- get element at index i, reassign; loop code block runs onces every iteration
...
function uniqueQuadratic(words) {
  ...
  for (
    ...
    for (
      ...
      const comparing = words[c];   // 2 * n^2 = O(n^2) -- get element words[c], reassign to comparing; nest loop (*n)
